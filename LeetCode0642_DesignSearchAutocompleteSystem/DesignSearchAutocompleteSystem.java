package LeetCode0642_DesignSearchAutocompleteSystem;
...
public class DesignSearchAutocompleteSystem {
    ...
    public void insert(Trie t, String s, int times) {
        for(int i = 0; i < s.length(); i++) {
            if(t.branches[int_(s.charAt(i))] == null) t.branches[int_(s.charAt(i))] = new Trie();
    ...
    public List<Node> lookup(Trie t, String s) {
        ...
        for(int i = 0; i < s.length(); i++) {
            if(t.branches[int_(s.charAt(i))] == null) return new ArrayList<Node>();
            t = t.branches[int_(s.charAt(i))];
        }
        traverse(s, t, list);
        return list;
